...
template<size_t H,size_t W> class Screen ;                                                    // 这个声明必要因下面输入/输出运算符而需要
template<size_t H,size_t W> istream &operator >> ( istream &in , Screen<H,W> &screen ) ;    // 这个声明需要是因为需要在Screen类中声明友元
